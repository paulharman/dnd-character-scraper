...
class EnhancedSpellcastingCalculator(RuleAwareCalculator, ICachedCalculator):
    ...
    def calculate(self, character_data: Dict[str, Any], context: CalculationContext) -> CalculationResult:
        ...
        try:
            ...
            is_caster = bool(spellcasting_classes)
            
            # Build result - only include spellcasting data for actual casters
            result_data = {
    ...
    def _calculate_spellcasting(self, character_data: Dict[str, Any], rule_version) -> SpellcastingData:
        ...
        if not spellcasting_classes:
            # No spellcasting - return minimal data
        ...
        # Debug logging for spell save DC calculation
        logger.info(f"Spell save DC calculation: 8 + {proficiency_bonus} + {spellcasting_modifier} = {spell_save_dc} (ability: {primary_ability})")
    ...
    def _create_non_caster_data(self) -> SpellcastingData:
        """Create minimal spellcasting data for non-spellcasters."""
        return SpellcastingData(
            spellcasting_ability=None,
    ...
    def _get_spellcasting_modifier(self, character_data: Dict[str, Any], ability_name: str) -> int:
        """Get spellcasting modifier for the specified ability including bonuses."""
        ...
        if ability_scores and ability_name in ability_scores:
            ...
            score = ability_scores[ability_name].get('score', 10)
            logger.info(f"Found {ability_name} score {score}, modifier {modifier} in new data structure")
            return modifier
        ...
        modifier = DnDMath.ability_modifier(effective_score)
        logger.info(f"{ability_name}: effective score {effective_score}, modifier {modifier}")
        return modifier

    def _get_effective_ability_score(self, character_data: Dict[str, Any], ability_name: str) -> int:
        ...
        logger.info(f"Found {len(feat_choices)} feat choices for {ability_name}")

        for choice in feat_choices:
            logger.debug(f"Processing choice: subType={choice.get('subType')}, optionValue={choice.get('optionValue')}")
            if choice.get('subType') == 5:  # Ability score choices
                option_value = choice.get('optionValue')
                logger.debug(f"Found ability score choice with option value {option_value}")
                bonus = self._get_ability_bonus_from_choice_option(option_value, ability_name)
                if bonus > 0:
                    total_bonus += bonus
                    logger.debug(f"Found +{bonus} {ability_name} bonus from choice option {option_value}")
                else:
                    logger.debug(f"Option value {option_value} does not provide bonus to {ability_name}")
    ...
    def _get_ability_bonus_from_choice_option(self, option_value: int, ability_name: str) -> int:
        ...
        ability_bonus_map = {
            # +2 bonuses (from ASI/feat selections)
            6260: ("constitution", 2),    # From the raw data: "Choose +2 to Constitution, Intelligence, or Wisdom"
            6261: ("intelligence", 2),
            6262: ("wisdom", 2),
            # +1 bonuses (from background/feat selections)
            6263: ("constitution", 1),    # From the raw data: "Choose +1 to Constitution, Intelligence, or Wisdom"
            6264: ("intelligence", 1),    # Confirmed from Sage background
            6265: ("wisdom", 1),
            6266: ("charisma", 1),
