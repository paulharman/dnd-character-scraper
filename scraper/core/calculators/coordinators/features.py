...
class FeaturesCoordinator(ICoordinator):
    ...
    def coordinate(self, raw_data: Dict[str, Any], context: CalculationContext) -> CalculationResult:
        ...
        try:
            ...
            # Cross-deduplicate feats and class features (keep the better version)
            feats, class_features = self._cross_deduplicate_features(feats, class_features)
            
            # Deduplicate remaining feats (D&D Beyond API can provide same feat with different IDs)
            feats = self._deduplicate_feats(feats)
            
            # Clean feat names (remove level prefixes like "4:")
            feats = self._clean_feat_names(feats)
    ...
    def _deduplicate_class_features(self, class_features: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Deduplicate class features that appear multiple times due to D&D Beyond API structure.
        
        The D&D Beyond API provides the same feature in multiple places:
        - Class definition features
        - Subclass definition features  
        - Character's actual class features
        - Feat references
        
        Args:
            class_features: List of class features to deduplicate
            
        Returns:
            Deduplicated list of class features
        """
        ...
        for feature in class_features:
            ...
            source_name = feature.get('source_name', '').strip()
            is_subclass = feature.get('is_subclass_feature', False)
            
            if not name:
                continue
                
            # Clean the name to handle level prefixes like "4: Weapon Mastery"
            clean_name = name.lower()
            import re
            clean_name = re.sub(r'^\d+:\s*', '', clean_name)
            
            # Create unique key based on cleaned name and level only
            # This deduplicates identical features from class vs subclass definitions
            # but preserves legitimate multi-level progressions
            key = f"{clean_name}_{level_required}"
            
            # If we haven't seen this feature yet, add it
            if key not in feature_map:
                feature_map[key] = feature
            else:
                # If we've seen it, keep the more complete version
                existing = feature_map[key]
                current = feature
                
                # Score features based on completeness (prefer more detailed descriptions)
                existing_score = self._score_feature_completeness(existing)
                current_score = self._score_feature_completeness(current)
                
                existing_subclass = existing.get('is_subclass_feature', False)
                current_subclass = current.get('is_subclass_feature', False)
                
                if current_score > existing_score:
                    feature_map[key] = current
                    self.logger.debug(f"Replaced feature '{name}' with more complete version (existing: subclass={existing_subclass}, score={existing_score}) -> (current: subclass={current_subclass}, score={current_score})")
                else:
                    # Keep existing feature but preserve subclass information if the current one is a subclass feature
                    if current_subclass and not existing_subclass:
                        existing['is_subclass_feature'] = True
                        existing['source_name'] = current.get('source_name', existing.get('source_name'))
                        self.logger.debug(f"Kept existing feature '{name}' but marked as subclass feature (source: {existing.get('source_name')})")
                    else:
                        self.logger.debug(f"Kept existing feature '{name}' (more complete) (existing: subclass={existing_subclass}, score={existing_score}) vs (current: subclass={current_subclass}, score={current_score})")
        
        deduplicated = list(feature_map.values())
        
        # Clean the feature names in the final output to remove level prefixes
        import re
        for feature in deduplicated:
            name = feature.get('name', '')
            if name and re.match(r'^\d+:\s*', name):
                # Remove level prefix from the actual feature name
                clean_name = re.sub(r'^\d+:\s*', '', name)
                feature['name'] = clean_name
                self.logger.debug(f"Cleaned feature name: '{name}' -> '{clean_name}'")
        
        # Sort by level required, then by name for consistent output
        deduplicated.sort(key=lambda f: (f.get('level_required', 1), f.get('name', '')))
        
        deduplicated_count = len(deduplicated)
        if original_count > deduplicated_count:
            self.logger.info(f"Scraper:  Deduplicated {original_count} class features to {deduplicated_count} unique features")
        
        return deduplicated
    
    def _cross_deduplicate_features(self, feats: List[Dict[str, Any]], class_features: List[Dict[str, Any]]) -> tuple:
        """
        Cross-deduplicate feats and class features, keeping the better version.
        
        Sometimes D&D Beyond provides the same ability as both a feat and a class feature.
        We compare them and keep whichever version is more complete/accurate.
        
        Args:
            feats: List of feats
            class_features: List of class features
            
        Returns:
            Tuple of (filtered_feats, filtered_class_features)
        """
        if not feats or not class_features:
            return feats, class_features
        
        # Create a set of class feature names for quick lookup
        class_feature_names = set()
        for feature in class_features:
            name = feature.get('name', '').strip().lower()
            # Clean name of level prefixes
            import re
            clean_name = re.sub(r'^\d+:\s*', '', name)
            class_feature_names.add(clean_name)
        
        # Cross-deduplicate by comparing feats with class features
        filtered_feats = []
        filtered_class_features = []
        features_to_remove = set()  # Track class features to remove
        removed_count = 0
        improved_count = 0
        
        for feat in feats:
            feat_name = feat.get('name', '').strip().lower()
            # Clean feat name of level prefixes  
            import re
            clean_feat_name = re.sub(r'^\d+:\s*', '', feat_name)
            
            if clean_feat_name in class_feature_names:
                # For multi-level progressions like Weapon Mastery, prefer class features
                # as they show the proper level-by-level advancement
                matching_features = [f for f in class_features 
                                   if re.sub(r'^\d+:\s*', '', f.get('name', '').strip().lower()) == clean_feat_name]
                
                if matching_features:
                    # Check if we have multiple levels of the same class feature
                    feature_levels = [cf.get('level_required', 1) for cf in matching_features]
                    has_multiple_levels = len(set(feature_levels)) > 1
                    
                    if has_multiple_levels:
                        # Multi-level progression - prefer class features to show progression
                        self.logger.debug(f"Removed redundant feat '{feat.get('name', '')}' (multi-level class feature progression exists)")
                        removed_count += 1
                    else:
                        # Single level - compare quality
                        feat_score = self._score_feature_completeness(feat)
                        best_class_feature_score = max(self._score_feature_completeness(cf) for cf in matching_features)
                        
                        if feat_score > best_class_feature_score:
                            # Feat is better - keep it and mark class features for removal
                            filtered_feats.append(feat)
                            # Mark the inferior class features for removal
                            for cf in matching_features:
                                features_to_remove.add(id(cf))
                            improved_count += 1
                            self.logger.debug(f"Kept superior feat '{feat.get('name', '')}' over inferior class feature(s)")
                        else:
                            # Class feature is better - remove feat
                            self.logger.debug(f"Removed redundant feat '{feat.get('name', '')}' (equivalent class feature exists)")
                            removed_count += 1
                else:
                    # No matching features found somehow, keep the feat
                    filtered_feats.append(feat)
            else:
                # No matching class feature, keep the feat
                filtered_feats.append(feat)
        
        # Filter out marked class features
        filtered_class_features = [cf for cf in class_features if id(cf) not in features_to_remove]
        
        if removed_count > 0:
            self.logger.info(f"Scraper:  Removed {removed_count} redundant feats (equivalent class features exist)")
        if improved_count > 0:
            self.logger.info(f"Scraper:  Replaced {improved_count} inferior class features with superior feat versions")
        
        return filtered_feats, filtered_class_features
    
    def _deduplicate_feats(self, feats: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Deduplicate feats that appear multiple times due to D&D Beyond API structure.
        
        The D&D Beyond API can provide the same feat with different IDs when feat
        definitions are updated or when there are multiple versions.
        
        Args:
            feats: List of feats to deduplicate
            
        Returns:
            Deduplicated list of feats
        """
        if not feats:
            return feats
        ...
        for feat in feats:
            ...
            # Use cleaned name as key for deduplication
            key = clean_name
            
            # If we haven't seen this feat yet, add it
            if key not in feat_map:
                feat_map[key] = feat
            else:
                # If we've seen it, keep the more complete version
                existing = feat_map[key]
                current = feat
                
                # Score feats based on completeness (prefer more detailed descriptions)
                existing_score = self._score_feature_completeness(existing)
                current_score = self._score_feature_completeness(current)
                
                if current_score > existing_score:
                    feat_map[key] = current
                    self.logger.debug(f"Replaced feat '{name}' with more complete version")
                else:
                    self.logger.debug(f"Kept existing feat '{name}' (more complete)")
        
        deduplicated = list(feat_map.values())
        
        # Clean the feat names in the final output to remove level prefixes
        import re
        for feat in deduplicated:
            name = feat.get('name', '')
            if name and re.match(r'^\d+:\s*', name):
                # Remove level prefix from the actual feat name
                clean_name = re.sub(r'^\d+:\s*', '', name)
                feat['name'] = clean_name
                self.logger.debug(f"Cleaned feat name: '{name}' -> '{clean_name}'")
        
        # Sort by name for consistent output
        deduplicated.sort(key=lambda f: f.get('name', ''))
        
        deduplicated_count = len(deduplicated)
        if original_count > deduplicated_count:
            self.logger.info(f"Scraper:  Deduplicated {original_count} feats to {deduplicated_count} unique feats")
        
        return deduplicated
    
    def _clean_feat_names(self, feats: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Clean feat names by removing level prefixes like "4: Weapon Mastery".
        
        Args:
            feats: List of feats to clean
            
        Returns:
            List of feats with clean names
        """
        if not feats:
            return feats
        
        # Clean the feat names to remove level prefixes
        import re
        for feat in feats:
            name = feat.get('name', '')
            if name and re.match(r'^\d+:\s*', name):
    ...
    def _score_feature_completeness(self, feature: Dict[str, Any]) -> int:
        """
        Score a feature based on its completeness for deduplication purposes.
        ...
        # Base score for having the feature
        score += 1
        
        # Score for description length and quality
        description = feature.get('description', '')
        if description:
            score += min(len(description) // 100, 10)  # Up to 10 points for description length
            
            # Bonus for more specific/accurate text
            if 'another kind' in description.lower():
                score += 5  # Prefer "another kind" over "two kinds" 
            if 'chosen weapon choices' in description.lower():
                score += 3  # Prefer "chosen weapon choices" over "weapon choices"
        
        snippet = feature.get('snippet', '')
        if snippet and not description:  # Only count snippet if no description
            score += min(len(snippet) // 100, 5)  # Up to 5 points for snippet length
        
        # Score for metadata completeness
        if feature.get('level_required'):
            score += 1
        if feature.get('source_name'):
            score += 1
        # Note: Removed is_subclass_feature scoring as this is now handled via preservation during deduplication
        
        # Score for limited use information
        limited_use = feature.get('limited_use')
        if limited_use and isinstance(limited_use, dict):
            score += 2
            if limited_use.get('max_uses'):
                score += 1
            if limited_use.get('reset_type'):
                score += 1
