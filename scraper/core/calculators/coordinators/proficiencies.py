...
from ..proficiency import EnhancedProficiencyCalculator
...
class ProficienciesCoordinator(ICoordinator):
    ...
    def __init__(self, config_manager=None):
        ...
        self.enhanced_calculator = EnhancedProficiencyCalculator()
    ...
    def coordinate(self, raw_data: Dict[str, Any], context: CalculationContext) -> CalculationResult:
        ...
        try:
            ...
            try:
                self.logger.debug("Using enhanced proficiency calculator")
                ...
                if enhanced_result.status == CalculationStatus.COMPLETED:
                    self.logger.info("Successfully used enhanced proficiency calculator")
                    # Enhanced calculator now returns data in coordinator format, no transformation needed
                    return enhanced_result
                else:
                    self.logger.error(f"Enhanced calculator failed with errors: {enhanced_result.errors}")
                    raise Exception(f"Enhanced calculator failed: {enhanced_result.errors}")
                    
            except Exception as e:
                self.logger.error(f"Enhanced calculator failed: {str(e)}")
                raise
    ...
    def _transform_enhanced_proficiency_result(self, enhanced_result: CalculationResult, raw_data: Dict[str, Any], context: CalculationContext) -> CalculationResult:
        """Transform enhanced proficiency calculator result to coordinator format."""
        try:
            enhanced_data = enhanced_result.data
            
            # Extract enhanced calculator results and transform to coordinator format
            result_data = {
                'skill_proficiencies': enhanced_data.get('skill_proficiencies', []),
                'saving_throw_proficiencies': enhanced_data.get('saving_throw_proficiencies', []),
                'tool_proficiencies': enhanced_data.get('tool_proficiencies', []),
                'gaming_set_proficiencies': enhanced_data.get('gaming_set_proficiencies', []),
                'musical_instrument_proficiencies': enhanced_data.get('musical_instrument_proficiencies', []),
            ...
            self.logger.error(f"Error transforming enhanced proficiency result: {str(e)}")
            raise
    ...
    def _format_output(self, data: ProficienciesData) -> Dict[str, Any]:
        ...
        return {
            ...
            "gaming_set_proficiencies": getattr(data, 'gaming_set_proficiencies', []),
            "musical_instrument_proficiencies": getattr(data, 'musical_instrument_proficiencies', []),
