...
from ..spellcasting_calculator import EnhancedSpellcastingCalculator
...
class SpellcastingCoordinator(ICoordinator):
    ...
    def __init__(self, config_manager=None):
        ...
        self.enhanced_calculator = EnhancedSpellcastingCalculator()
    ...
    def coordinate(self, raw_data: Dict[str, Any], context: CalculationContext) -> CalculationResult:
        ...
        try:
            ...
            try:
                self.logger.debug("Using enhanced spellcasting calculator")
                # Pass raw data directly to preserve choices structure for ability score bonuses
                enhanced_result = self.enhanced_calculator.calculate(raw_data, context)
                
                if enhanced_result.status == CalculationStatus.COMPLETED:
                    self.logger.info("Successfully used enhanced spellcasting calculator")
                    return self._transform_enhanced_spellcasting_result(enhanced_result, raw_data, context)
                else:
            ...
            # Comprehensive spellcasting calculation
            self.logger.debug("Using comprehensive spellcasting calculation")
    ...
    def _transform_enhanced_spellcasting_result(self, enhanced_result: CalculationResult, raw_data: Dict[str, Any], context: CalculationContext) -> CalculationResult:
        """Transform enhanced spellcasting calculator result to coordinator format."""
        try:
            enhanced_data = enhanced_result.data
            
            # Extract enhanced calculator results and transform to coordinator format
            result_data = {
                'is_spellcaster': enhanced_data.get('is_spellcaster', False),
                'spellcasting_ability': enhanced_data.get('spellcasting_ability'),
                'spell_save_dc': enhanced_data.get('spell_save_dc'),
                'spell_attack_bonus': enhanced_data.get('spell_attack_bonus'),
                'spell_slots': enhanced_data.get('spell_slots', [0] * 9),
                'pact_slots': enhanced_data.get('pact_slots', 0),
                'pact_slot_level': enhanced_data.get('pact_slot_level', 0),
                'caster_level': enhanced_data.get('caster_level', 0),
                'spell_slot_breakdown': enhanced_data.get('spell_slot_breakdown', {}),
                'spell_counts': enhanced_data.get('spell_counts', {}),
            ...
            self.logger.error(f"Error transforming enhanced spellcasting result: {str(e)}")
            raise
    ...
    def _get_ability_modifiers(self, raw_data: Dict[str, Any], context: CalculationContext) -> Dict[str, int]:
        """Get ability modifiers from context or calculate from raw data with bonuses."""
        ...
        # Try new data structure first (abilities.ability_modifiers)
        abilities = raw_data.get('abilities', {})
        if 'ability_modifiers' in abilities:
            ability_modifiers = abilities['ability_modifiers']
            self.logger.debug(f"Found ability modifiers in new data structure: {ability_modifiers}")
        ...
        if ability_scores:
            ...
            if ability_modifiers:
                self.logger.debug(f"Found ability modifiers from ability_scores: {ability_modifiers}")
                return ability_modifiers

        # Fallback: calculate from effective ability scores (base + bonuses)
        ability_modifiers = {}
        self.logger.debug("Calculating ability modifiers from effective ability scores")
        ...
        for ability_name in ability_names:
            ...
            self.logger.debug(f"{ability_name}: effective score {effective_score}, modifier {modifier}")

        self.logger.debug(f"Final ability modifiers: {ability_modifiers}")
        return ability_modifiers

    def _get_effective_ability_score(self, raw_data: Dict[str, Any], ability_name: str) -> int:
        """Get effective ability score including bonuses from choices (feats/backgrounds)."""
        ...
        ability_id = ability_id_map.get(ability_name)
        if ability_id:
            for stat in stats:
        ...
        for choice in feat_choices:
            if choice.get('subType') == 5:  # Ability score choices
                option_value = choice.get('optionValue')
                bonus = self._get_ability_bonus_from_choice_option(option_value, ability_name)
                if bonus > 0:
                    total_bonus += bonus
                    self.logger.debug(f"Found +{bonus} {ability_name} bonus from choice option {option_value}")

        effective_score = base_score + total_bonus
        self.logger.debug(f"{ability_name}: base {base_score} + bonus {total_bonus} = effective {effective_score}")
        return effective_score

    def _get_ability_bonus_from_choice_option(self, option_value: int, ability_name: str) -> int:
        """Map D&D Beyond choice option values to ability bonuses."""
        # D&D Beyond option values for ability score improvements
        # These are commonly used values - may need expansion
        ability_bonus_map = {
            # Common ability score improvement option values
            6260: ("strength", 1),
            6261: ("dexterity", 1),
            6262: ("constitution", 1),
            6263: ("intelligence", 1),
            6264: ("intelligence", 1),  # Specific to Sage background
            6265: ("wisdom", 1),
            6266: ("charisma", 1),
            # Add more mappings as discovered
        }

        if option_value in ability_bonus_map:
            bonus_ability, bonus_amount = ability_bonus_map[option_value]
            if bonus_ability == ability_name:
                return bonus_amount
