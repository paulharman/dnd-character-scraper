...
class ConfigManager:
    ...
    def load_config_file(self, file_path: Path) -> Dict[str, Any]:
        """Load and parse a configuration file (YAML or JSON) with enhanced error handling."""
    ...
    def load_json_file(self, file_path: Path) -> Dict[str, Any]:
        """Load and parse a JSON configuration file with enhanced error handling."""
        try:
            ...
            if file_size > 10 * 1024 * 1024:  # 10MB limit
                raise ValueError(f"Configuration file too large ({file_size} bytes): {file_path}")
            ...
            if not isinstance(data, dict):
                raise ValueError(f"Configuration file must contain a JSON object, got {type(data).__name__}: {file_path}")
                
            return data
            
        except (OSError, IOError) as e:
            raise RuntimeError(f"I/O error loading {file_path}: {e}")
        except Exception as e:
            raise RuntimeError(f"Unexpected error loading {file_path}: {e}")

    def load_yaml_file(self, file_path: Path) -> Dict[str, Any]:
        """Load and parse a YAML configuration file with enhanced error handling."""
        try:
            if not file_path.exists():
                raise FileNotFoundError(f"Configuration file not found: {file_path}")
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                try:
                    ...
                    raise ValueError(f"Invalid YAML structure in {file_path}: {e}")
                except yaml.scanner.ScannerError as e:
                    raise ValueError(f"YAML syntax error in {file_path}: {e}")
            ...
            if not isinstance(data, dict):
                raise ValueError(f"Configuration file must contain a YAML dictionary, got {type(data).__name__}: {file_path}")
    ...
    def load_environment_config(self, environment: str) -> Dict[str, Any]:
        """Load environment-specific configuration (supports YAML and JSON)."""
        # Try YAML first, then JSON
        for ext in ['.yaml', '.yml', '.json']:
            env_config_path = self.paths.config_dir / "environments" / f"{environment}{ext}"
            if env_config_path.exists():
                try:
                    return self.load_config_file(env_config_path)
                except Exception as e:
                    self.logger.warning(f"Failed to load environment config {env_config_path}: {e}")
                    continue
        
        # Return empty config if no environment file found
        self.logger.debug(f"No environment configuration found for '{environment}'")
        return {}
    ...
    def get_app_config(self) -> AppConfig:
        ...
        if self._app_config is None:
            try:
                ...
                # Try to create a minimal default configuration
                try:
                    self._app_config = self._create_fallback_config()
                    self.logger.warning("Using fallback configuration due to loading errors")
                except Exception as fallback_error:
                    self.logger.critical(f"Failed to create fallback configuration: {fallback_error}")
                    raise RuntimeError(f"Configuration loading failed and fallback creation failed: {e}") from e
    ...
    def _load_organized_config(self) -> Dict[str, Any]:
        ...
        config_data = self.load_config_file(self.paths.main_config)
        
        # Load and merge scraper config
        if self.paths.scraper_config.exists():
            scraper_config = self.load_config_file(self.paths.scraper_config)
        ...
        if self.paths.discord_config.exists():
            discord_config = self.load_config_file(self.paths.discord_config)
        ...
        if self.paths.parser_config.exists():
            parser_config = self.load_config_file(self.paths.parser_config)
    ...
    def get_constants_config(self) -> GameConstantsConfig:
        ...
        if self._constants_config is None:
            config_data = self.load_config_file(self.paths.constants_config)
    ...
    def get_rules_config(self, rule_version: str) -> RuleSpecificConfig:
        ...
        if rule_version == "2014":
            if self._rules_2014_config is None:
                config_data = self.load_config_file(self.paths.rules_2014)
            ...
            if self._rules_2024_config is None:
                config_data = self.load_config_file(self.paths.rules_2024)
    ...
    def _create_fallback_config(self) -> AppConfig:
        ...
        fallback_data = {
            "project": {
                "name": "D&D Beyond Character Scraper",
                "version": "1.0.0"
            },
            "api": {
                ...
                "timeout": 30,
                "max_retries": 3,
                "user_agent": "D&D Beyond Character Scraper"
            },
            "output": {
                "verbose": False,
                "format": "json",
            ...
            "features": {
                "enhanced_change_detection": True,
        ...
        return AppConfig(**fallback_data)
