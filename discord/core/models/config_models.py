...
class EnhancedNotificationConfig:
    """Configuration for enhanced notification system."""
    
    webhook_url: str
    username: str = "D&D Beyond Monitor"
    enabled_change_types: Optional[Set[str]] = None
    priority_filters: Optional[Dict[str, str]] = None
    detail_levels: Optional[Dict[str, str]] = None
    ...
    discord_detail_level: str = "detailed"
    change_log_retention_days: int = 365
    change_log_rotation_size_mb: int = 10
    avatar_url: Optional[str] = None
    max_changes_per_notification: int = 200
    min_priority: ChangePriority = ChangePriority.MEDIUM
    ...
    def __post_init__(self):
        ...
        if self.enabled_change_types is None:
            self.enabled_change_types = {
                'feats', 'spells', 'level', 'abilities', 'equipment', 'inventory'
            }
        elif isinstance(self.enabled_change_types, list):
            self.enabled_change_types = set(self.enabled_change_types)
        
        if self.priority_filters is None:
            self.priority_filters = {
                'minimum_priority': 'MEDIUM',
                'feat_priority': 'HIGH',
                'spell_priority': 'MEDIUM'
            }
        
        if self.detail_levels is None:
            self.detail_levels = {
                'discord': 'BRIEF',
                'console': 'DETAILED',
                'log': 'COMPREHENSIVE'
    ...
    def validate(self) -> List[str]:
        ...
        for key, priority in self.priority_filters.items():
            if priority not in valid_priorities:
                errors.append(f"Invalid priority '{priority}' for {key}")
        
        valid_detail_levels = {'BRIEF', 'DETAILED', 'COMPREHENSIVE'}
        for key, level in self.detail_levels.items():
            if level not in valid_detail_levels:
                errors.append(f"Invalid detail level '{level}' for {key}")
...
def convert_notification_config(old_config) -> EnhancedNotificationConfig:
    """Convert old notification config to enhanced notification config."""
    # Handle different types of old config
    if hasattr(old_config, 'webhook_url'):
        webhook_url = old_config.webhook_url
    elif isinstance(old_config, dict):
        webhook_url = old_config.get('webhook_url', '')
    else:
        webhook_url = str(old_config)
    
    # Extract username
    username = "D&D Beyond Monitor"  # default
    if hasattr(old_config, 'username'):
        username = old_config.username
    elif isinstance(old_config, dict) and 'username' in old_config:
        username = old_config['username']
    
    # Extract other properties if they exist
    enabled_change_types = set()
    if hasattr(old_config, 'enabled_change_types'):
        try:
            enabled_change_types = set(old_config.enabled_change_types)
        except (TypeError, AttributeError):
            # Handle Mock objects or other non-iterable types
            enabled_change_types = set()
    elif isinstance(old_config, dict) and 'enabled_change_types' in old_config:
        enabled_change_types = set(old_config['enabled_change_types'])
    
    priority_filters = {}
    if hasattr(old_config, 'priority_filters'):
        try:
            priority_filters = dict(old_config.priority_filters)
        except (TypeError, AttributeError):
            # Handle Mock objects or other non-dict types
            priority_filters = {}
    elif isinstance(old_config, dict) and 'priority_filters' in old_config:
        priority_filters = dict(old_config['priority_filters'])
    
    # Extract additional attributes
    avatar_url = getattr(old_config, 'avatar_url', None)
    max_changes_per_notification = getattr(old_config, 'max_changes_per_notification', 200)
    min_priority = getattr(old_config, 'min_priority', ChangePriority.MEDIUM)
    include_groups = getattr(old_config, 'include_groups', None)
    exclude_groups = getattr(old_config, 'exclude_groups', None)
    rate_limit_requests_per_minute = getattr(old_config, 'rate_limit_requests_per_minute', 10)
    rate_limit_burst = getattr(old_config, 'rate_limit_burst', 5)
    timezone = getattr(old_config, 'timezone', "UTC")
    send_summary_for_multiple = getattr(old_config, 'send_summary_for_multiple', True)
    delay_between_messages = getattr(old_config, 'delay_between_messages', 1.0)
    
    # Convert sets if needed
    if include_groups and not isinstance(include_groups, set):
        try:
            include_groups = set(include_groups)
        except (TypeError, AttributeError):
            include_groups = None
    
    if exclude_groups and not isinstance(exclude_groups, set):
        try:
            exclude_groups = set(exclude_groups)
        except (TypeError, AttributeError):
            exclude_groups = None
    
    # Create enhanced config with defaults for missing values
    enhanced_config = EnhancedNotificationConfig(
        webhook_url=webhook_url,
        username=username,
        enabled_change_types=enabled_change_types if enabled_change_types else None,
        priority_filters=priority_filters if priority_filters else None,
        avatar_url=avatar_url,
        max_changes_per_notification=max_changes_per_notification,
        ...
        timezone=timezone,
        send_summary_for_multiple=send_summary_for_multiple,
