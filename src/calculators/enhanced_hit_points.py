...
class EnhancedHitPointsCalculator(RuleAwareCalculator, ICachedCalculator):
    ...
    def _calculate_hit_points(self, character_data: Dict[str, Any], rule_version) -> HitPointsData:
        ...
        # Get hit point info if available
        hp_info = character_data.get('hitPointInfo', {})
        current_hp = hp_info.get('current', 0)
        max_hp = hp_info.get('maximum', 0)
        temp_hp = hp_info.get('temp', 0)
        
        # If we have direct HP values, use them; otherwise calculate
        if max_hp > 0:
        ...
        hp_breakdown = {
            'base_hp': base_hp,
            'con_bonus': con_modifier * total_level,
            'class_bonus': self._get_class_hp_bonus(character_data),
            'feat_bonus': self._get_feat_hp_bonus(character_data),
            'item_bonus': self._get_item_hp_bonus(character_data),
            'misc_bonus': self._get_misc_hp_bonus(character_data)
        }
    ...
    def _get_class_hp_bonus(self, character_data: Dict[str, Any]) -> int:
        ...
        # Check for class-based HP bonuses (like Draconic Sorcerer)
        classes = character_data.get('classes', [])
        bonus = 0
        ...
        for class_data in classes:
            ...
            # Draconic Sorcerer gets +1 HP per level
